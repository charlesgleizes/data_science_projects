...
def get_url(bbox, height, width):
    ...
    # Maximum WIDTH and HEIGHT is 10_000. We rescale to get max image size for the bbox height and width
    rescaler = 10_000 / max(height * 5, width * 5)
    height = height * 5 * rescaler
    width = width * 5 * rescaler

    return url_start + bbox + url_width + str(int(width)) + url_height + str(int(height))